...
to initiate-new-cfps [ _x ]
  ...
  ask boats [send-message msg]
  set initiator-cfps lput list id _x initiator-cfps
end

;;;;;;;;;;;;;;;
to read-cfps-answers
  let finished_cfps []
  foreach initiator-cfps [ _cfps ->
    let _id first _cfps
    let _whale last _cfps
    let lmsgs get-lmsgs-key mailbox "conversation_id" _id
    if not empty? lmsgs[
      set mailbox remove-from-lmgs mailbox lmsgs
      let proposals get-lmsgs-key lmsgs "performative" "proposal"
      ifelse not empty? proposals [
        let proposals-senders map[m -> get-value-msg m "sender"] proposals
        let winner map[m -> get-value-msg m "sender"](get-min-msg proposals)
        let loosers remove-from-lmgs proposals-senders winner

        let accept-msg create-message "accept-proposal"
        set accept-msg set-value-msg accept-msg "conversation_id" _id
        set accept-msg set-value-msg accept-msg "content" _whale
        foreach winner [ w ->
          ask w [send-message accept-msg]
        ]

        let msg create-message "reject-proposal"
        set msg set-value-msg msg "conversation_id" _id
        foreach loosers [ looser ->
          ask looser [send-message msg]
        ]
      ][
        set failed-cfps lput _whale failed-cfps
      ]
...
to check-new-cfps
  let new_cfps get-lmsgs-key mailbox "performative" "cfps"

  if not empty? new_cfps [
    set mailbox remove-from-lmgs mailbox new_cfps
  ]

  foreach new_cfps [ _msg ->
    let _id get-value-msg _msg "conversation_id"
    let _whale  first get-value-msg _msg "content"
    let sender get-value-msg _msg "sender"

    let distance-to-whale distance _whale
    ifelse distance-to-whale <= ratio-sensor
    [
      ;; Create message and set attributes
      let msg create-message "proposal"
      set msg set-value-msg msg "conversation_id" _id
      set msg set-value-msg msg "content" distance-to-whale
      ;; Send message to sender
      ask sender [send-message msg]
...
to-report create-message [performative]
  let performative-value list "performative" performative
  let sender list "sender" self
  report list performative-value sender
end
...
to-report get-min-msg [lmsgs]
  let values map [_msg -> get-value-msg _msg "content"] lmsgs
  let min_value min values
  report filter [_msg -> (get-value-msg _msg "content") = min_value] lmsgs
end

to-report remove-from-lmgs [lmgs lmgs-to-remove]
  report filter [msg -> not member? msg lmgs-to-remove] lmgs
end
